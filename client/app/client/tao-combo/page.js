...
function Page() {
  ...
  const onSubmit = async (data) => {
    ...
    formData.append("stage_id", parsedState(hallId).id); // Wait for API to fix
    files.forEach((file) => {
                ...
                <TabPanel key={"decor"}>
                  ...
                  >
                    ...
                    <div className="w-2/3 h-full  flex flex-col gap-3 relative">
                      <div className="flex gap-3 items-center h-full relative">
                        <div className="shrink-0 w-1 h-full bg-gold absolute left-0 top-0 bottom-0"></div>
                        <span className="text-xl font-bold ml-4">Sân khấu</span>
                      </div>
                      <span className="text-sm font-normal text-left">
                ...
                <TabPanel key={"weddingCake"}>
                  ...
                  >
                    ...
                    <div className="w-2/3 h-full  flex flex-col gap-3 relative">
                      ...
                      <span className="text-sm font-normal text-left">
                        Lựa chọn chiếc bánh cưới ưng ý nhất cho tiệc cưới của
                        quý khách
                      </span>
                      <div className="w-full flex flex-wrap gap-4">
                        {products &&
                  ...
                  <motion.div animate={{ opacity: 1 }}>
                    ...
                    <div className="flex flex-col w-full gap-4 mt-5">
                      {Object.keys(drawerData).map((key) => {
                        if (!Array.isArray(drawerData[key]) && !drawerData[key])
                          return null;
                ...
                >
                  <div className="flex flex-col flex-1">
                    ...
                    <p className="text-white font-semibold text-xl mt-2">
                      {total ? `${total.toLocaleString("vn-VN")} VNĐ` : "0 VNĐ"}
                    </p>
                  </div>
                  <ChevronDownIcon
