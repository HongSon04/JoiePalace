    ...
    return (
        <div className="min-h-screen p-8 flex flex-col items-center justify-between">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-6xl w-full ">
                {listItemPackages.map((pkg) => (
                    <div key={pkg.id} className={`overflow-y-hidden  p-3 rounded-lg flex flex-col items-center space-y-4 ${pkg.premium ? 'border border-gold' : ''}`}>
                        ...
                        <p className="text-center text-2xl font-semibold uppercase z-20">{(pkg.priceform) / 1000000} - {(pkg.priceto) / 1000000} TRIỆU VND</p>
                        <p className="text-center z-20 ">{pkg.description}</p>
                        ...
                        <div className="flex justify-center items-center gap-4">
                            <button className="flex justify-center items-center gap-2 bg-gold text-white px-4 py-2 rounded-lg font-semibold ">
                                LIÊN HỆ NGAY <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M12.0254 4.94189L17.0837 10.0002L12.0254 15.0586" stroke="white" stroke-width="1.5" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round" />
...
export default WeddingPackagesForUser;
