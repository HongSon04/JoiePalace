...
function CategoriesTable() {
  ...
  const sortedItems = React.useMemo(() => {
    if (!selectedCategory || !selectedCategory.childrens) return [];

    return [...selectedCategory.childrens].sort((a, b) => {
  ...
  }, [sortDescriptor, selectedCategory]);
        ...
        return (
          <div className="relative flex justify-center items-center gap-2">
            <Dropdown>
              ...
              <DropdownMenu>
                <DropdownItem>
                  <Link href={`/admin/quan-ly-danh-muc/${id}/${item.id}`}>
                    Xem chi tiết
                  </Link>
                </DropdownItem>
                <DropdownItem
