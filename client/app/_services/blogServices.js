...
export const createCategoryBlog = async (dataToSend) => {
  const response = await axios.post(API_CONFIG.BLOGS.CREATE, dataToSend);

  if (response.status !== 200 && response.status !== 201) {
    throw new Error("Có lỗi khi danh mục mới");
  }
